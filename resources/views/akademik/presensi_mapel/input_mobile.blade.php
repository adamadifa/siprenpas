...
@section('title', 'Input Presensi - Sipren')
@section('header-title', 'Input Presensi')
    ...
    <div class="presensi-container">
        <form action="{{ route('presensi-mapel.store') }}" method="POST">
                    ...
                    <div class="meeting-meta-list">
                        ...
                        <div class="meta-item">
                            ...
                            <div>
                                ...
                                <span class="meta-value">{{ $jadwal->guru->karyawan->nama_lengkap }}</span>
                            </div>
                        </div>
                        <!-- Hari, Tanggal -->
                        <div class="meta-item">
                            ...
                            <div>
                                ...
                                <span class="meta-value">{{ $jadwal->hari }}, {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d M Y') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="meta-row-split">
                            ...
                            <div class="meta-item">
                                ...
                                <div>
                                    ...
                                    <span class="meta-value">{{ substr($jadwal->jam_mulai, 0, 5) }} - {{ substr($jadwal->jam_selesai, 0, 5) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div style="margin-bottom: 20px;">
                <div class="card border-0" style="border-radius: 10px; background: var(--surface); box-shadow: var(--shadow-sm); overflow: hidden; border: none;">
                    <div class="card-body p-3">
                        ...
                        <textarea name="materi" class="materi-textarea" rows="3" placeholder="Tuliskan materi yang disampaikan pada pertemuan ini..."></textarea>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body p-3">
                            ...
                            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    @if (!empty($s->foto) && Storage::disk('public')->exists('photos/pendaftaran/' . $s->foto))
                                        <img src="{{ asset('storage/photos/pendaftaran/' . $s->foto) }}" class="student-photo" alt="{{ $s->nama_lengkap }}">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        <span class="student-id">{{ $s->id_siswa }}</span>
                                    </div>
                                </div>
                            ...
                            <div class="presence-options">
                                <div class="presence-option">
                                    <input type="radio" class="status-h" name="status[{{ $s->id_siswa }}]" id="h_{{ $s->id_siswa }}" value="h" checked>
                                    <label class="presence-label" for="h_{{ $s->id_siswa }}">Hadir</label>
                                </div>
                                <div class="presence-option">
                                    <input type="radio" class="status-i" name="status[{{ $s->id_siswa }}]" id="i_{{ $s->id_siswa }}" value="i">
                                    <label class="presence-label" for="i_{{ $s->id_siswa }}">Izin</label>
                                </div>
                                <div class="presence-option">
                                    <input type="radio" class="status-s" name="status[{{ $s->id_siswa }}]" id="s_{{ $s->id_siswa }}" value="s">
                                    <label class="presence-label" for="s_{{ $s->id_siswa }}">Sakit</label>
                                </div>
                                <div class="presence-option">
                                    <input type="radio" class="status-a" name="status[{{ $s->id_siswa }}]" id="a_{{ $s->id_siswa }}" value="a">
                                    <label class="presence-label" for="a_{{ $s->id_siswa }}">Alpha</label>
                                </div>
                            </div>
                            ...
                            <div class="note-input-wrapper">
                                ...
                                <input type="text" name="keterangan[{{ $s->id_siswa }}]" class="note-input" placeholder="Tambah catatan keterangan...">
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div style="margin-top: 24px; margin-bottom: 40px;">
                <button type="submit" class="btn btn-save-presensi btn-action">
