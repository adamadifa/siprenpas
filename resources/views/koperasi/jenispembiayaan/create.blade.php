<form action="{{ route('jenispembiayaan.store') }}" id="formPembiayaan" method="POST">
    @csrf
    <x-input-with-icon-label icon="ti ti-barcode" label="Kode Jenis Pembiayaan" name="kode_pembiayaan" required="true" />
    <x-input-with-icon-label icon="ti ti-file-description" label="Nama Jenis Pembiayaan" name="jenis_pembiayaan" required="true" />
    <x-input-with-icon-label icon="ti ti-calculator" label="Persentase (%)" name="persentase" required="true" />
    ...
    <div class="form-group mt-3">
        <button class="btn btn-primary w-100 shadow-sm d-flex align-items-center justify-content-center gap-2" type="submit" id="btnSimpan" style="background-color: #064e3b; border-color: #064e3b">
            ...
            <span class="fw-bold">Simpan Data Jenis Pembiayaan</span>
        </button>
    </div>
</form>
