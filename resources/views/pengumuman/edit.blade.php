<form action="{{ route('pengumuman.update', $pengumuman->id) }}" method="POST" id="formPengumuman">
    @csrf
    @method('PUT')
    <x-input-with-icon icon="ti ti-file-description" label="Judul Pengumuman" name="judul"
    ...
    <x-select label="Kategori" name="kategori_id" :data="$kategori" key="id" textShow="nama_kategori"
        selected="{{ old('kategori_id', $pengumuman->kategori_id) }}" />
    @error('kategori_id')
        <div class="invalid-feedback d-block">{{ $message }}</div>
    @enderror
    ...
    <div class="row">
        ...
        <div class="col-lg-6 col-sm-12 col-md-12">
            <x-input-with-icon icon="ti ti-map-pin" label="Lokasi" name="lokasi"
    ...
    <x-textarea-label name="isi" label="Isi Pengumuman" value="{{ old('isi', $pengumuman->isi) }}" />
    @error('isi')
        <div class="invalid-feedback d-block">{{ $message }}</div>
    @enderror
